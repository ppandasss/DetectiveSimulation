...
//
//         float xpos = x + m_face->glyph->bitmap_left * scale;
//        float ypos = y - (m_face->glyph->bitmap.rows - m_face->glyph->bitmap_top) * scale;
//        float w = m_face->glyph->bitmap.width * scale;
//        float h = m_face->glyph->bitmap.rows * scale;
//
...
//        x += (m_face->glyph->advance.x >> 6) * scale;
//
//        GLenum err = glGetError();
//        if (err != GL_NO_ERROR) {
...
int main()
{
    ...
    // find path to font
    std::string font_name = "Assets/Fonts/WD.ttf";
    if (font_name.empty())
    {
